...
class Variables extends Abstract_model {
	...
	public $table			= "recruitment.core_variables";
	public $pkey			= "var_id";
	...
	public $fromClause 		= "recruitment.core_variables AS variables";
	...
	function validate() {
		if($this->actionType == 'CREATE') {
			//do something
			$this->record['var_id'] = $this->generate_id('recruitment','core_variables','var_id');
		}else {
			//do something
		}
		return true;
	...
	function get_var($var_name) {
	    ...
	    $sql = "SELECT var_value FROM recruitment.core_variables WHERE var_name = '".$var_name."'";       
	...
	function set_var($var_name, $var_value) {
	    $sql = "UPDATE recruitment.core_variables
	...
	function set_theme($user_name, $var_name, $var_value) {
	    $sql = "SELECT COUNT(1) as total FROM recruitment.core_variables
	                WHERE var_component = ?
	                AND var_name = ?";
